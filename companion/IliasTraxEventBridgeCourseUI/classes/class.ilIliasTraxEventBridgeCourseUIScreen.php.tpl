...
class ilIliasTraxEventBridgeCourseUIScreen
{
    ...
    private function renderShell(string $content, int $courseRefId, string $courseTitle): string
    {
        ...
        return $this->styles()
            . '<div id="itxeb-course-ui-screen">'
            . '<h1>' . $this->esc($title) . '</h1><p>Configuration xAPI depuis l’objet cours' . ($courseRefId > 0 ? ' — course_ref_id ' . $this->esc((string) $courseRefId) : '') . '</p>'
            . $content
            . '</div>';
    }
    ...
    private function getCurrentUserId(): int
    {
        ...
        return isset($GLOBALS['ilUser']) && is_object($GLOBALS['ilUser']) && method_exists($GLOBALS['ilUser'], 'getId')
            ? (int) $GLOBALS['ilUser']->getId()
            : 0;
    }
    ...
    {
        return '<style>'
            . '#itxeb-course-ui-screen{margin:0;padding:0}'
            . '#itxeb-course-ui-screen h1{font-size:24px;margin:.2rem 0 .3rem}'
            . '#itxeb-course-ui-screen h2{font-size:18px;margin:1rem 0 .5rem}'
            . '#itxeb-course-ui-screen .itxeb-cui-section{margin-bottom:18px}'
            . '#itxeb-course-ui-screen .itxeb-cui-alert{padding:.65rem .8rem;margin:.4rem 0 .9rem;border:1px solid #bce8f1;background:#eef8fc;border-radius:4px}'
            . '#itxeb-course-ui-screen .itxeb-cui-error{border-color:#ebccd1;background:#f2dede;color:#a94442}'
            . '#itxeb-course-ui-screen .itxeb-cui-success{border-color:#d6e9c6;background:#dff0d8;color:#3c763d}'
            . '#itxeb-course-ui-screen .itxeb-cui-table{width:100%;border-collapse:collapse;background:#fff}'
            ...
            . '#itxeb-course-ui-screen .itxeb-cui-table th{background:#f7f7f7}'
            . '#itxeb-course-ui-screen .itxeb-cui-table-wrapper{overflow-x:auto;background:#fff;border:1px solid #ddd;border-radius:4px}'
            . '#itxeb-course-ui-screen .itxeb-cui-resource-table{min-width:1050px}'
            . '</style>';
    }
}
