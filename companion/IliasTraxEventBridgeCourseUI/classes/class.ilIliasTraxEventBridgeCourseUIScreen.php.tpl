...
class ilIliasTraxEventBridgeCourseUIScreen
{
    ...
    private function renderView(array $course, string $cmd): string
    {
        ...
        return $this->renderCourseSummary($course) . $this->renderConfigForm($course) . $this->renderDashboardPreferencesForm($course) . $this->renderBulkActions((int) ($course['course_ref_id'] ?? 0));
    }
    ...
    private function renderDashboard(array $course): string
    {
        ...
        $html = '<section class="itxeb-cui-section"><h2>Tableau de bord du cours</h2><p>Vue synthétique des traces xAPI générées par les ressources du cours.</p>'
        ...
        if (!empty($widgets['technical_status'])) {
    ...
    private function renderEnabledWithoutTraceResources(array $dashboard): string
    {
        ...
        $html = '<section class="itxeb-cui-section"><h3>Ressources activées sans trace</h3>';
        if (count($rows) === 0) {
            return $html . '<p><em>Aucune ressource activée sans trace pour la période et le filtre sélectionnés.</em></p></section>';
        }

        $html .= '<p>Ces ressources sont activées xAPI dans la configuration du cours, mais aucune trace locale n’a été générée sur la période sélectionnée.</p>'
        ...
        foreach ($rows as $stats) {
            $html .= '<tr><td><strong>' . $this->esc((string) ($stats['title'] ?? '')) . '</strong><br><small>' . $this->esc((string) ($stats['path'] ?? '')) . '</small></td>'
                ...
                . '<td><span class="itxeb-signal">activée sans trace</span></td></tr>';
        }
        return $html . '</tbody></table></div></section>';
    }
    ...
    private function dashboardWidgetDefinitions(): array
    {
        return [
            ...
            'enabled_without_trace' => 'Ressources activées sans trace',
            'technical_status' => 'État technique local',
        ];
    }
