# Validation V0.12.1 — consolidation du compagnon UI

## Objectif de la validation

Valider que la V0.12.1 conserve le comportement fonctionnel de la V0.12.0 tout en installant le compagnon `IliasTraxEventBridgeCourseUI` depuis des templates consolidés, sans rejouer les patchers successifs historiques.
...
```text
Branche : v0.12.1-consolidation-ui-companion
Version : 0.12.1-dev
```

La version finale sera passée à `0.12.1` après validation complète.
...
Mode  : V0.12.1 consolidated templates
PHP syntax check
...
- le wrapper existe ;
- la branche affichée est `v0.12.1-consolidation-ui-companion` ;
- la version est `0.12.1-dev` ;
...
Résultat V0.12.1 : OK. Les erreurs visibles dans les extraits de validation étaient antérieures aux tests et ne correspondent pas à une régression de cette version.
...
La V0.12.1 peut être finalisée si :

- l'installation du compagnon est OK ;
- l'audit V0.12.1 est OK ;
...
- le comportement fonctionnel est équivalent à la V0.12.0 ;
...
Remarques : validation fonctionnelle V0.12.1 OK. Installation depuis templates consolidés, audit OK, vues Tableau de bord / Analyse / Expert / Configuration OK, exports CSV et PDF OK, pas de nouvelle erreur bloquante observée dans les logs.
```
