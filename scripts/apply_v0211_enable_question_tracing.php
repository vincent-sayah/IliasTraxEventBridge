...
foreach ([$plugin, $factory, $router, $extractor] as $required) {
    if (!is_file($required)) {
        fwrite(STDERR, "Fichier absent: $required\n");
        exit(1);
    }
}
...
if (strpos($s, 'public function createStatementsFromEventRecord(array $record): array') === false) {
    ...
    $pos = strpos($s, $marker);
    if ($pos === false) {
        fwrite(STDERR, "Point insertion createStatementsFromEventRecord introuvable\n");
        exit(1);
    }
    $s = substr($s, 0, $pos) . $createMany . substr($s, $pos);
    echo "PATCH: createStatementsFromEventRecord\n";
} else {
    echo "OK: createStatementsFromEventRecord\n";
}
...
if (strpos($s, 'private function createQuestionResultStatements(array $record): array') === false) {
    ...
    $pos = strpos($s, $marker);
    if ($pos === false) {
        fwrite(STDERR, "Point insertion méthodes questions introuvable\n");
        exit(1);
    }
    $s = substr($s, 0, $pos) . $questionMethods . substr($s, $pos);
    echo "PATCH: méthodes questions\n";
} else {
    echo "OK: méthodes questions\n";
}
...
set_version($plugin, '0.21.1-dev');
echo "V0.21.1 appliquee : traces par question de test activees.\n";
