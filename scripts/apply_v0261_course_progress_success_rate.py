...
# -*- coding: utf-8 -*-
"""
V0.26.1 — Taux de réussite du cours dans la synthèse pédagogique.
...
BACKUP_DIR = Path(tempfile.gettempdir()) / ("itxeb_v0261_backup_" + datetime.now().strftime("%Y%m%d%H%M%S"))
...
PEDAGOGICAL_SYNTHESIS_METHOD = r'''    /** @param array<string,mixed> $dashboard */
    ...
    {
        // ITXEB V0.26.1 course progress success rate in synthesis.
...
def patch_summary(content):
    if "ITXEB V0.26.1 course progress success rate" in content:
        fail("V0.26.1 semble déjà appliquée dans LrsCourseSummary")
...
def patch_screen(content):
    if "ITXEB V0.26.1 course progress success rate in synthesis" in content:
        fail("V0.26.1 semble déjà appliquée dans CourseUIScreen")
    content = replace_method(content, "renderPedagogicalSynthesis", PEDAGOGICAL_SYNTHESIS_METHOD)
    return content


def lint_php_memory(name, content):
    temp_dir = Path(tempfile.mkdtemp(prefix="itxeb_v0261_lint_"))
...
print("V0.26.1 préflight: lecture fichiers")
...
print("V0.26.1 préflight: vérification structure")
...
print("V0.26.1 préflight: calcul patchs mémoire")
summary2 = patch_summary(summary)
screen2 = patch_screen(screen)
main_plugin2 = patch_version(main_plugin, "0.26.1-dev", "plugin principal")
...
print("V0.26.1 préflight: contrôles mémoire")
checks = [
    ...
    ("screen marker", "ITXEB V0.26.1 course progress success rate in synthesis", screen2),
    ("screen label", "Réussite du cours", screen2),
    ("version main", "0.26.1-dev", main_plugin2),
...
print("V0.26.1 préflight: lint PHP mémoire")
...
print("V0.26.1 préflight OK: écriture fichiers")
...
print("V0.26.1 contrôle PHP fichiers écrits")
for path in [SUMMARY_FILE, SCREEN_TEMPLATE, MAIN_PLUGIN, COMPANION_PLUGIN_TEMPLATE, LIVE_SCREEN, LIVE_COMPANION_PLUGIN]:
    lint_php_file(path)

print("V0.26.1 appliquée : taux de réussite du cours ajouté dans Synthèse pédagogique.")
print("Versions : plugin principal 0.26.1-dev / compagnon 0.8.45")
