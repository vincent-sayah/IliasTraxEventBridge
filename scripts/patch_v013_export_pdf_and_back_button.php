<?php
/**
 * V0.13 - correction export PDF + retour intuitif au cours.
 *
 * Contexte : la page routée fonctionne, mais le bouton PDF généré par l'écran
 * historique utilise une URL du type :
 *   cmd=showDashboard&itxeb_cui_cmd=exportCourseDashboardPdf
 * Le router lisait cmd=showDashboard puis écrasait itxeb_cui_cmd avec
 * showCourseDashboard, donc l'export PDF n'était jamais déclenché.
 *
 * Ce patch :
 * - passe itxebcui en 0.3.5 ;
 * - priorise itxeb_cui_cmd lorsqu'il est présent dans l'URL ;
 * - préserve exportCourseDashboardPdf / exportCourseExpertCsv / saveCourseTracking ;
 * - ajoute une barre visible "Retour au contenu du cours" en haut du suivi xAPI.
 *
 * À lancer depuis la racine du plugin EventHook IliasTraxEventBridge :
 * php scripts/patch_v013_export_pdf_and_back_button.php
 */

function itxeb_export_fail(string $message): void
{
    fwrite(STDERR, "ERREUR: {$message}\n");
    exit(1);
}

function itxeb_export_write(string $file, string $content): void
{
    $dir = dirname($file);
    if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) {
        itxeb_export_fail("création répertoire impossible: {$dir}");
    }
    if (file_put_contents($file, $content) === false) {
        itxeb_export_fail("écriture impossible: {$file}");
    }
    echo "WRITE: {$file}\n";
}
...
if (!is_file($root . '/plugin.php') || !is_dir($root . '/classes')) {
    itxeb_export_fail('lance ce script depuis la racine du plugin EventHook IliasTraxEventBridge.');
}
...
if (substr($root, -strlen($eventHookSuffix)) !== $eventHookSuffix) {
    itxeb_export_fail("chemin plugin principal inattendu: {$root}");
}
...
itxeb_export_write($companionTemplate . '/plugin.php.tpl', $pluginPhp);
itxeb_export_write($companionTemplate . '/classes/class.ilIliasTraxEventBridgeCourseUIRouterGUI.php.tpl', $router);
if (is_dir($companionInstalled)) {
    itxeb_export_write($companionInstalled . '/plugin.php', $pluginPhp);
    itxeb_export_write($companionInstalled . '/classes/class.ilIliasTraxEventBridgeCourseUIRouterGUI.php', $router);
}
...
foreach ($files as $file) {
    ...
    if ($code !== 0) {
        itxeb_export_fail("syntaxe PHP invalide: {$file}");
    }
}

echo "\nCorrectif appliqué : version itxebcui 0.3.5 + export PDF routé + bouton retour cours.\n";
