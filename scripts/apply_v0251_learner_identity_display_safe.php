...
/**
 * V0.25.1 — Affichage nominatif des apprenants dans Analyse et Expert.
 *
 * Correctif sûr après V0.25.0 : remplace explicitement les méthodes d'affichage
 * sensibles au lieu de dépendre d'une chaîne trop exacte.
 */
...
$stamp = date('YmdHis');
$backupDir = sys_get_temp_dir() . '/itxeb_v0251_backup_' . $stamp;
...
function fail_v0251(string $message): void
{
    fwrite(STDERR, 'ERREUR: ' . $message . PHP_EOL);
    exit(1);
}

function read_file_v0251(string $path): string
{
    if (!is_file($path)) {
        fail_v0251('fichier introuvable: ' . $path);
    }
    $content = file_get_contents($path);
    if (!is_string($content)) {
        fail_v0251('lecture impossible: ' . $path);
    }
    return $content;
}

function backup_file_v0251(string $path, string $backupDir): void
{
    if (!is_dir($backupDir) && !mkdir($backupDir, 0777, true) && !is_dir($backupDir)) {
        fail_v0251('creation dossier backup impossible: ' . $backupDir);
    }
    if (is_file($path)) {
        $target = $backupDir . '/' . str_replace('/', '__', ltrim($path, '/'));
        if (!copy($path, $target)) {
            fail_v0251('backup impossible: ' . $path);
        }
    }
}

function write_file_v0251(string $path, string $content, string $backupDir): void
{
    backup_file_v0251($path, $backupDir);
    if (file_put_contents($path, $content) === false) {
        fail_v0251('ecriture impossible: ' . $path);
    }
}

function replace_once_v0251(string $content, string $search, string $replace, string $label): string
{
    ...
    if ($pos === false) {
        fail_v0251('point de remplacement introuvable: ' . $label);
    }
    return substr($content, 0, $pos) . $replace . substr($content, $pos + strlen($search));
}

/** @return array{0:int,1:int} */
function method_bounds_v0251(string $content, string $methodName): array
{
    $needle = 'private function ' . $methodName . '(';
    $start = strpos($content, $needle);
    if ($start === false) {
        fail_v0251('methode introuvable: ' . $methodName);
    }
    if (strpos($content, $needle, $start + 1) !== false) {
        fail_v0251('methode en double: ' . $methodName);
    }
    $brace = strpos($content, '{', $start);
    if ($brace === false) {
        fail_v0251('accolade introuvable: ' . $methodName);
    }
    ...
    fail_v0251('fin de methode introuvable: ' . $methodName);
}

function replace_method_v0251(string $content, string $methodName, string $newMethod): string
{
    [$start, $end] = method_bounds_v0251($content, $methodName);
    return substr($content, 0, $start) . rtrim($newMethod) . substr($content, $end);
}

function replace_in_method_v0251(string $content, string $methodName, callable $callback): string
{
    [$start, $end] = method_bounds_v0251($content, $methodName);
    $method = substr($content, $start, $end - $start);
    $patched = $callback($method);
    if (!is_string($patched) || $patched === '') {
        fail_v0251('patch methode invalide: ' . $methodName);
    }
    return substr($content, 0, $start) . $patched . substr($content, $end);
}

function patch_lrs_summary_v0251(string $content): string
{
    if (strpos($content, "'learner_identity' =>") === false) {
        $content = replace_once_v0251(
            $content,
...
PHP;
        $content = replace_once_v0251($content, $marker, $method . $marker, 'insertion actorDisplayName');
    }

    return $content;
}

function patch_screen_v0251(string $content): string
{
    // Corrige au passage la mauvaise quote HTML constatée dans le template V0.24.17.
    $content = str_replace('<div class= itxeb-dashboard-top-card"', '<div class="itxeb-dashboard-top-card"', $content);
    $content = str_replace("<div class= itxeb-dashboard-top-card\"", "<div class=\"itxeb-dashboard-top-card\"", $content);

    $newStrugglingLearners = <<<'PHP'
...
PHP;
    $content = replace_method_v0251($content, 'renderStrugglingLearners', $newStrugglingLearners);
...
PHP;
    $content = replace_method_v0251($content, 'renderExpert', $newExpert);

    $content = replace_in_method_v0251($content, 'sendExpertCsv', static function (string $method): string {
        if (strpos($method, "'learner_identity'") === false) {
            $method = replace_once_v0251(
                $method,
...
function patch_version_v0251(string $content, string $newVersion, string $label): string
{
    $updated = preg_replace("/\\$version\\s*=\\s*'[^']+';/", "\$version = '" . $newVersion . "';", $content, 1, $count);
    if (!is_string($updated) || $count !== 1) {
        fail_v0251('version introuvable: ' . $label);
    }
    return $updated;
}

function lint_v0251(string $path): void
{
    if (!is_file($path)) {
        return;
    }
    ...
    if ($rc !== 0) {
        fail_v0251('lint PHP KO: ' . $path);
    }
}

foreach ([$summaryFile, $screenTemplate, $mainPlugin, $companionPluginTemplate, $liveScreen, $liveCompanionPlugin] as $file) {
    if (is_file($file)) {
        backup_file_v0251($file, $backupDir);
    }
}

write_file_v0251($summaryFile, patch_lrs_summary_v0251(read_file_v0251($summaryFile)), $backupDir);
write_file_v0251($screenTemplate, patch_screen_v0251(read_file_v0251($screenTemplate)), $backupDir);
if (is_file($liveScreen)) {
    write_file_v0251($liveScreen, patch_screen_v0251(read_file_v0251($liveScreen)), $backupDir);
}

write_file_v0251($mainPlugin, patch_version_v0251(read_file_v0251($mainPlugin), '0.25.1-dev', 'plugin principal'), $backupDir);
write_file_v0251($companionPluginTemplate, patch_version_v0251(read_file_v0251($companionPluginTemplate), '0.8.39', 'plugin compagnon template'), $backupDir);
if (is_file($liveCompanionPlugin)) {
    write_file_v0251($liveCompanionPlugin, patch_version_v0251(read_file_v0251($liveCompanionPlugin), '0.8.39', 'plugin compagnon live'), $backupDir);
}
...
foreach ([$summaryFile, $screenTemplate, $liveScreen] as $file) {
    ...
    $content = read_file_v0251($file);
    if (strpos($content, 'learner_identity') === false) {
        fail_v0251('controle final KO learner_identity absent: ' . $file);
    }
    if (strpos($content, 'private function renderStrugglingLearners') !== false) {
        [$start, $end] = method_bounds_v0251($content, 'renderStrugglingLearners');
        $method = substr($content, $start, $end - $start);
        if (strpos($method, 'anonymous_id') !== false || strpos($method, 'Vue anonymisée') !== false) {
            fail_v0251('controle final KO anonymisation encore presente: ' . $file);
        }
    }
    if (strpos($content, '<th>User ID</th><th>Apprenant</th><th>Verbe</th>') === false && strpos($content, 'class.ilIliasTraxEventBridgeLrsCourseSummary') === false) {
        fail_v0251('controle final KO colonne Apprenant Expert absente: ' . $file);
    }
}

echo 'V0.25.1 appliquee : identite apprenant non anonymisee dans Analyse et Expert.' . PHP_EOL;
