...
class ilIliasTraxEventBridgeConfigGUI extends ilPluginConfigGUI
{
    ...
    private function renderState(): string
    {
        ...
        foreach (['Plugin actif'=>$this->config->isEnabled()?'oui':'non','Mode debug'=>$this->config->isDebugEnabled()?'oui':'non','Génération xAPI locale'=>$this->config->isLocalXapiGenerationEnabled()?'oui':'non','Cron plugin'=>$this->config->isCronEnabled()?'activé':'désactivé'] as $k=>$v) {
    ...
    private function renderDenyLogSupervision(): string
    {
        ...
        $html = '<section class="itxeb-section"><h2>Diagnostic des traces refusées V0.8</h2>'
            . '<div class="itxeb-alert"><strong>Objectif :</strong> expliquer pourquoi une trace xAPI n’a pas été générée. Les refus sont issus de <code>evnt_evhk_itxeb_dlog</code> et ne bloquent jamais la navigation ILIAS ni le cron.</div>'
        ...
        if (count($rows) === 0) {
            return $html . '<p><em>Aucun refus journalisé pour le moment.</em></p></section>';
        }
    ...
    private function saveConfig(): void
    {
        ...
        foreach (['TraxEndpoint'=>'trax_endpoint','TraxUsername'=>'trax_username','XapiVersion'=>'xapi_version','IliasBaseUrl'=>'ilias_base_url'] as $m=>$k) {
            $this->config->{'set'.$m}($this->postString($k));
        }
    ...
    private function testTraxConnection(): void
    {
        ...
        if ($r->isSuccess()) {
            $this->success('Connexion TRAX réussie : ' . $r->getShortMessage());
        } elseif (class_exists('ilUtil') && method_exists('ilUtil', 'sendFailure')) {
            ilUtil::sendFailure('Connexion TRAX échouée : ' . $r->getShortMessage(), true);
        }
        $this->ctrl->redirect($this, 'configure');
    }
    ...
    private function sendGenerated(): void
    {
        ...
        if ($r['success']) {
            $this->success((string)$r['message']);
        } elseif (class_exists('ilUtil') && method_exists('ilUtil', 'sendFailure')) {
            ilUtil::sendFailure((string)$r['message'], true);
        }
        $this->ctrl->redirect($this, 'configure');
    }
    ...
    {
        $this->success($this->outbox->resetFailedToGenerated() . ' statement(s) failed réinitialisé(s).');
        $this->ctrl->redirect($this, 'configure');
    }
    ...
    private function renderOutbox(): string
    {
        ...
        if (count($rows) === 0) {
            return $html . '<p><em>Aucun statement xAPI généré pour le moment.</em></p></section>';
        }
    ...
    private function renderRecentEvents(): string
    {
        ...
        if (count($rows) === 0) {
            return $html . '<p><em>Aucun événement journalisé pour le moment.</em></p></section>';
        }
    ...
    private function styles(): string
    {
        return '<style>'
            ...
            . '.itxeb-deny-table{min-width:1450px}.itxeb-deny-table th:nth-child(1),.itxeb-deny-table td:nth-child(1){width:165px}.itxeb-deny-table th:nth-child(2),.itxeb-deny-table td:nth-child(2){width:190px}.itxeb-deny-table th:nth-child(3),.itxeb-deny-table td:nth-child(3){width:330px}.itxeb-deny-table th:nth-child(4),.itxeb-deny-table td:nth-child(4){width:160px}.itxeb-deny-table th:nth-child(5),.itxeb-deny-table td:nth-child(5){width:140px}.itxeb-deny-summary{max-width:1200px}'
