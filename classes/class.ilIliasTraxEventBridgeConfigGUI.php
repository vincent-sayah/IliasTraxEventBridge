...
class ilIliasTraxEventBridgeConfigGUI extends ilPluginConfigGUI
{
    ...
    {
        $html = $this->styles() . '<div class="itxeb-page"><h1>IliasTraxEventBridge — V0.8</h1>'
    ...
    private function renderCourseTrackingAccess(): string
    {
        ...
        return '<section class="itxeb-section itxeb-course-admin-access"><h2>Configuration xAPI par cours</h2>'
            . '<div class="itxeb-alert"><strong>V0.7.1 :</strong> la configuration est disponible depuis <code>Cours &gt; Paramètres &gt; Suivi xAPI</code>. Cette section admin reste disponible pour ouvrir directement un cours par <code>ref_id</code>.</div>'
    ...
    {
        $html = '<section class="itxeb-section"><h2>Configuration TRAX / cron</h2><div class="itxeb-alert"><strong>Important :</strong> cette section pilote les paramètres techniques. Le diagnostic des traces refusées doit rester désactivé en exploitation courante et être activé uniquement pendant une analyse ciblée.</div>';
    ...
    private function renderSendActions(): string
    {
        ...
        foreach (['generated'=>$this->outbox->countByStatus('generated'),'failed'=>$this->outbox->countByStatus('failed'),'retry épuisé'=>$this->outbox->countRetryExhausted($this->config->getMaxRetry()),'taille batch'=>$this->config->getBatchSize(),'max_retry'=>$this->config->getMaxRetry()] as $k=>$v) {
            $html .= '<span><strong>'.$this->esc((string)$k).' :</strong> '.$this->esc((string)$v).'</span>';
        }
    ...
    private function renderAdminDashboard(): string
    {
        ...
        $html = '<section class="itxeb-section"><h2>Supervision outbox</h2>';
        ...
        foreach ($rows as $r) {
            ...
            if (count($recentDiagnostics) < 12) {
                $recentDiagnostics[] = ['id'=>(string)($r['id']??''),'status'=>(string)($r['status']??''),'event_type'=>(string)($r['event_type']??''),'obj_type'=>(string)($r['obj_type']??''),'family'=>$family,'source'=>$source!==''?$source:$recordSource,'deduplication_key'=>$deduplicationKey];
            }
        }
    ...
    private function renderDenyLogSupervision(): string
    {
        ...
        $sourceCounts = [];
        $eventTypeCounts = [];
        foreach ($this->denyLog->findRecent(200) as $row) {
            $this->countValue($reasonCounts, (string) ($row['reason'] ?? ''));
            $this->countValue($sourceCounts, (string) ($row['source_table'] ?? ''));
            $this->countValue($eventTypeCounts, (string) ($row['event_type'] ?? ''));
        }
        ...
        $html = '<section class="itxeb-section"><h2>Diagnostic des traces refusées V0.8</h2>'
            . '<div class="itxeb-alert"><strong>Objectif :</strong> expliquer pourquoi une trace xAPI n’a pas été générée. État actuel : <span class="itxeb-badge ' . $statusClass . '">' . $this->esc($statusText) . '</span>. À laisser désactivé en exploitation courante sur une plateforme volumineuse.</div>'
            . '<p class="itxeb-actions"><a class="btn btn-default" href="' . $this->esc($this->ctrl->getLinkTarget($this, 'clearDenyLog')) . '">Purger le diagnostic des traces refusées</a></p>'
            . '<div class="itxeb-dashboard-block itxeb-deny-summary"><h3>Synthèse des refus</h3><div class="itxeb-summary itxeb-ops-summary"><span><strong>Total refus :</strong> ' . $this->esc((string) $this->denyLog->countAll()) . '</span></div></div>';

        $html .= $this->renderCounterBlock('Motifs de refus', $reasonCounts)
        ...
        $html .= '<div class="itxeb-table-wrapper"><table class="std itxeb-events itxeb-deny-table"><thead><tr><th>ID / date</th><th>Motif</th><th>Événement</th><th>Contexte</th><th>Source</th><th>Payload</th></tr></thead><tbody>';
        foreach ($rows as $r) {
            $reason = (string) ($r['reason'] ?? '');
            $html .= '<tr><td class="itxeb-nowrap">#' . $this->esc((string) ($r['id'] ?? '')) . '<br><span class="itxeb-date">' . $this->esc((string) ($r['created_at'] ?? '')) . '</span></td>'
                . '<td><span class="itxeb-badge ' . $this->reasonBadgeClass($reason) . '">' . $this->esc($reason) . '</span></td>'
                . '<td>' . $this->esc((string) ($r['event_type'] ?? '')) . '<br><span class="itxeb-date">' . $this->esc((string) ($r['component'] ?? '')) . '<br>' . $this->esc((string) ($r['event_name'] ?? '')) . '</span></td>'
                . '<td class="itxeb-object">user ' . $this->esc((string) ($r['user_id'] ?? '')) . '<br>course ' . $this->esc((string) ($r['course_ref_id'] ?? '')) . '<br>ref ' . $this->esc((string) ($r['ref_id'] ?? '')) . '<br>obj ' . $this->esc((string) ($r['obj_id'] ?? '')) . '<br>' . $this->esc((string) ($r['obj_type'] ?? '')) . '</td>'
                . '<td class="itxeb-object">' . $this->esc((string) ($r['source_table'] ?? '')) . '<br>#' . $this->esc((string) ($r['source_id'] ?? '')) . '</td>'
    ...
    private function renderDiagnosticRows(array $rows): string
    {
        ...
        foreach ($rows as $r) {
            $html .= '<tr><td class="itxeb-nowrap">#' . $this->esc($r['id']) . '</td><td><span class="itxeb-badge ' . $this->statusBadgeClass($r['status']) . '">' . $this->esc($r['status']) . '</span></td><td>' . $this->esc($r['event_type']) . '<br><span class="itxeb-date">' . $this->esc($r['obj_type']) . '</span></td><td>' . $this->esc($r['family']) . '</td><td>' . $this->esc($r['source']) . '</td><td><code class="itxeb-code-inline">' . $this->esc($r['deduplication_key']) . '</code></td></tr>';
        }
    ...
    private function renderFailureRows(array $rows): string
    {
        ...
        foreach ($rows as $r) {
            $html .= '<tr><td class="itxeb-nowrap">#' . $this->esc((string) ($r['id'] ?? '')) . '</td><td><span class="itxeb-badge ' . $this->statusBadgeClass((string) ($r['status'] ?? '')) . '">' . $this->esc((string) ($r['status'] ?? '')) . '</span></td><td>' . $this->esc((string) ($r['event_type'] ?? '')) . '<br><span class="itxeb-date">' . $this->esc((string) ($r['obj_type'] ?? '')) . '</span></td><td>' . $this->esc((string) ($r['retry_count'] ?? '0')) . ' / ' . $this->esc((string) ($r['max_retry'] ?? $this->config->getMaxRetry())) . '</td><td class="itxeb-error">' . $this->esc((string) ($r['last_error'] ?? '')) . '</td></tr>';
        }
        return $html . '</tbody></table></div></div>';
    }
    ...
    private function renderOutbox(): string
    {
        ...
        foreach ($rows as $r) {
            ...
            $html .= '<tr><td class="itxeb-nowrap">#'.$this->esc((string)($r['id'] ?? '')).'<br><span class="itxeb-date">'.$this->esc((string)($r['created_at'] ?? '')).'</span><br><span class="itxeb-date">last try: '.$this->esc((string)($r['last_attempt_at'] ?? '')).'</span></td><td><span class="itxeb-badge '.$this->statusBadgeClass((string)($r['status'] ?? '')).'">'.$this->esc((string)($r['status'] ?? '')).'</span></td><td class="itxeb-nowrap">'.$this->esc((string)($r['retry_count'] ?? '0')).' / '.$this->esc((string)($r['max_retry'] ?? $this->config->getMaxRetry())).'</td><td class="itxeb-verb-cell"><div class="itxeb-verb">'.$this->esc((string)($r['verb_id'] ?? '')).'</div></td><td class="itxeb-object">user '.$this->esc((string)($r['user_id'] ?? '')).'<br>ref '.$this->esc((string)($r['ref_id'] ?? '')).'<br>obj '.$this->esc((string)($r['obj_id'] ?? '')).'<br>'.$this->esc((string)($r['obj_type'] ?? '')).'</td><td class="itxeb-wide">'.($err !== '' ? '<div class="itxeb-error">'.$this->esc($err).'</div>' : '').'<details><summary>Statement</summary><pre>'.$this->esc($this->formatPayload((string)($r['statement_json'] ?? ''))).'</pre></details></td></tr>';
        }
        return $html . '</tbody></table></div></section>';
    }
    ...
    private function renderRecentEvents(): string
    {
        ...
        foreach ($rows as $r) {
            $html .= '<tr><td class="itxeb-nowrap">#'.$this->esc((string)($r['id'] ?? '')).'<br><span class="itxeb-date">'.$this->esc((string)($r['created_at'] ?? '')).'</span></td><td><div class="itxeb-text-block">'.$this->esc((string)($r['component'] ?? '')).'<br><strong>'.$this->esc((string)($r['event_name'] ?? '')).'</strong></div></td><td class="itxeb-object">user '.$this->esc((string)($r['user_id'] ?? '')).'<br>ref '.$this->esc((string)($r['ref_id'] ?? '')).'<br>obj '.$this->esc((string)($r['obj_id'] ?? '')).'<br>'.$this->esc((string)($r['obj_type'] ?? '')).'</td><td class="itxeb-uri-cell"><div class="itxeb-uri itxeb-request-uri">'.$this->esc((string)($r['request_uri'] ?? '')).'</div></td><td class="itxeb-wide"><details><summary>Payload</summary><pre>'.$this->esc($this->formatPayload((string)($r['payload_json'] ?? ''))).'</pre></details></td></tr>';
        }
        return $html . '</tbody></table></div></section>';
    }
    ...
    private function styles(): string
    {
        return '<style>'
            ...
            . '.itxeb-outbox-table{min-width:1500px}.itxeb-log-table{min-width:1550px}.itxeb-diagnostic-table{min-width:1300px}.itxeb-failure-table{min-width:1050px}.itxeb-deny-table{min-width:1450px}.itxeb-deny-table th:nth-child(1),.itxeb-deny-table td:nth-child(1){width:165px}.itxeb-deny-table th:nth-child(2),.itxeb-deny-table td:nth-child(2){width:190px}.itxeb-deny-table th:nth-child(3),.itxeb-deny-table td:nth-child(3){width:330px}.itxeb-deny-table th:nth-child(4),.itxeb-deny-table td:nth-child(4){width:160px}.itxeb-deny-table th:nth-child(5),.itxeb-deny-table td:nth-child(5){width:140px}.itxeb-deny-summary{max-width:1200px}'
            ...
            . '@media (max-width:900px){.itxeb-state-table td:first-child,.itxeb-form-table td:first-child{width:auto;min-width:0;white-space:normal}.itxeb-outbox-table{min-width:1250px}.itxeb-log-table{min-width:1250px}.itxeb-diagnostic-table{min-width:1100px}.itxeb-deny-table{min-width:1200px}}</style>';
    }
}
